...
bool HalfEdgeMesh::AddFace(const std::vector<Vector3<float> > &verts) {
  ...
  // based on fö 1 and figure 6 in lab instructions 
  // Add the vertices of the face/triangle
  size_t V0, V1, V2;
  AddVertex(verts.at(0), V0);
  AddVertex(verts.at(1), V1);
  AddVertex(verts.at(2), V2);
  // Add all half-edge pairs
  size_t halfEdge01, halfEdge12, halfEdge20, pair0, pair1, pair3;
  AddHalfEdgePair(V0, V1, halfEdge01, pair0);
  AddHalfEdgePair(V1, V2, halfEdge12, pair1);
  AddHalfEdgePair(V2, V0, halfEdge20, pair3);
  // Connect inner ring
  // first 
  ...
  // normalized)
  Face myFace; 
  myFace.edge = halfEdge01;
  // facenormal vill ha index av vilket face som finns med i mFaces => behöver finnas i listan och den är sista facet som las till.  (-1 av size för index börjar på 0)
  mFaces.push_back(myFace);
  size_t indexOfTriangle = mFaces.size() - 1;
  f(indexOfTriangle).normal = FaceNormal(indexOfTriangle);

  // All half-edges share the same left face (previously added)
  // match with a the index of the wanted triangle That is the latest added. 
  e(halfEdge01).face = indexOfTriangle;
  e(halfEdge12).face = indexOfTriangle;
  e(halfEdge20).face = indexOfTriangle;
